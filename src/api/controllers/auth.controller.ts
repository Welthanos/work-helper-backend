...
export const registerUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (existingUser.rows.length > 0) return res.status(409).json({ message: 'Este e-mail já está em uso.' });
...
export const loginUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!user) return res.status(401).json({ message: 'As credenciais do usuário são inválidas.' });
